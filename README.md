...
**Warning: the Discord platform in general is not only famous for being packed with deviant people (doxxing, bullying and insults are considered as perfectly OK as long as you are "inclusive"), it's also a privacy nightmare.** There's no easy way, if any, for users to mass delete their own messages (moderators can lock them in read-only mode or ban you and cut off all access). Consider that **you loose any right with anything you publish there**. Have fun anyway...
