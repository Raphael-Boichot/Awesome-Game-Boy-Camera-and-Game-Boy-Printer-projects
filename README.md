...
- Non open source commercial devices because they can be dismantled to look inside.
- Compiled codes without the sources **ONLY** if they have historical / preservation value.

**Are not listed:** 
- Non primary sources (citations of citations, A.I. generated slop, etc.). I want the source !
- Undocumented enough / unintelligible projects. I'm quite good at guessing what people try to do but I have my limits.
- Projects / items that cannot be reproduced / bought. Like "look pals I did that" then diseappears from the internet without elaborating.
- Any information that requires being a logged member / app owner to see it. **If it's not public, it does not exist.**
- Any social media post by extension, with very few exceptions. They are just not the good support for long lasting information.
- The shitload of camera filters / apps for webcam to fake the Game Boy Camera rendering, not that it isn't interesting, it's just not the point here.

Some projets are probably yet listed in the [awesome-gbdev project](https://github.com/gbdev/awesome-gbdev) or some other cryptic listing not updated for eons but I don't care. I'm not affiliated with any online retro community. I'm maintaining this list updated for my own nerdy satisfaction only. Consider that I'm definitely bored with this niche topic when updates cease. Oh, and just to precise, I'm neither an A.I. neither native English speaker so this repository is probably full of typos.

Are you in a hurry to be addicted ? [Jump directly here !](https://github.com/Raphael-Boichot/Awesome-Game-Boy-Camera-and-Game-Boy-Printer-projects/blob/main/README.md#your-new-addiction---getting-started)

![](/Hall_of_fame.jpg)
Examples of what you can tinker entering the rabbit hole (Image credit: Raphaël BOICHOT)

# Game Boy Camera
## Flashable Cameras
- [Flashable Game Boy Camera](https://github.com/2BitWizard/GB_Mini_Camera) by 2BitWizard (mini size board) and a [comprehensive fork](https://github.com/Raphael-Boichot/GB_Mini_Camera) by Raphael-Boichot. This project is license free and compatible with the [Camera +](https://ko-fi.com/s/9457d1cc6e) and [Camera + Mini](https://ko-fi.com/s/a4d7bd649a) shells. Current version is 2.0 in the main project (option for dual ROM bank), 1.4 in the fork (single ROM bank).
- The [first project of flashable camera](https://github.com/HDR/Gameboy-Camera-Flashcart) Regular size board, dual ROM bank.
...
- [PicNRec AIO](https://shop.insidegadgets.com/product/gameboy-camera-picnrec-aio/), a combined all-in-one (AIO) PicNrec + flashable camera and the plain [Gameboy Camera Flash Cart](https://shop.insidegadgets.com/product/gameboy-camera-flash-cart/) (basically the AIO less the [PicNRec](https://shop.insidegadgets.com/product/gameboy-camera-picnrec/)), by InsideGadget (regular size board).
- [A MAC-GBD replica of the Game Boy Camera](https://github.com/krovma/pocket-free-cam/tree/master) by krovma. In its current state, hard to build without doing educated guesses.

## FRAM mod
- [FRAM camera mod](https://gbdev.gg8.se/forums/viewtopic.php?id=843) by famiac. Honestly, never saw any working unit but why not. I anyway advise increasing the 22 pF caps to at least 33 pF.

## Custom camera ROMs
- [Photo!](https://github.com/untoxa/gb-photo) by untoxa, most curated homebrew project which basically allows extended use of the camera sensor and Game Boy Printer / Game Boy Printer emulators. Also compatible with the [Megaduck](https://fr.wikipedia.org/wiki/Mega_Duck) (also known as Cougar Boy) via a [Game Boy Camera / Megaduck adapter](https://github.com/bbbbbr/gb_to_megaduck_cart_adapter) and a special [extension card for Game Boy Camera](https://github.com/Raphael-Boichot/Megaduck-to-game-boy-camera-adapter). Photo! is also [Game Gear](https://fr.wikipedia.org/wiki/Game_Gear) compatible via an [adpater board](https://github.com/untoxa/gamegear-gbcamera-adapter).
- [2bit PXLR Studio](https://github.com/HerrZatacke/2bit-pxlr-studio) by HerrZatacke, prototype of Photo! which offers a smooth experience due to basic but efficient menus. It allows sending images by the [Game Boy speaker](https://github.com/Raphael-Boichot/custom-camera-rom-beep-tone-converter) (because, why not).
...
- [Game Boy Camera recompilation](https://github.com/GB-Recomp/gbcamera), using [gb-recompiled](https://github.com/GB-Recomp/gb-recompiled), a "static recompiler" for original GameBoy ROMs that translates Z80 assembly directly into "portable, modern C code". Honestly the biggest fucking mess of obfuscated spaghetti code I ever saw. Fly, you fools !

## Analogue Pocket Frenzy
- [PocketRoll](https://github.com/Guillain-RDCDE/PocketRoll) by Guillain-RDCDE, Verilog mod for infinite camera roll for the Analogue Pocket !
...
- The [CMOS Holga](https://github.com/mupfdev/CMOS-Holga) by mupfdev. A DIY digital camera using the Mitsubishi M64282FP image sensor and a STM32. Apparently unrelated to this [old STM32 library](https://github.com/gdelazzari/STM32-GameBoy-Camera) by gdelazzari.
...
- [The first documented attempt to hack the sensor out of scientific community](https://web.archive.org/web/20060630011231/http://geocities.com/vjkemp/gbcam.htm) by Victor Kemp, 2001. First ever color image captured with RGB filters !
...
- [A project of autonomous driving robot from 2001 also](https://k-y-yoshino.sakura.ne.jp/w6/w6_top.html) by K.Y. Yoshino. Looks like a personal hobby project to what I understand despite the project being very advanced.
...
- A [2012 student project of autonomous driving robot](https://people.ece.cornell.edu/land/courses/ece4760/FinalProjects/s2011/bcc44_acl84_my259/bcc44_acl84_my259/index.html) by Cornell University students, using the Game Boy Camera sensor. Video [here](https://www.youtube.com/watch?v=s4di_hl58ug).
...
- [A parallel interface](http://zebu.www5.50megs.com/camera.htm) for the camera sensor, unspecified date or author, may be around 2011.
...
- The [Game Boy Camera dithering patterns](https://github.com/HerrZatacke/dither-pattern-gen) explained, by HerrZatacke, and a [color extension](https://github.com/MaddoScientisto/color-dither-gbfier), by MaddoScientisto.
...
- [gbc-webcam](https://github.com/wordandahalf/gbc-webcam) by wordandahalf, A color webcam made using the M64282FP silicon retina found in the Gameboy Camera (stuck in developement).

## Extract pictures from / manipulate SRAM backup
...
- [Game Boy Camera Fast Wifi Adapter](https://github.com/MattGrayYes/Game-Boy-Camera-Fast-Wifi-Adapter) by MattGrayYes, based on the [GBxCart](https://www.gbxcart.com/).
- [HDR image processing](https://github.com/seb-tourneux/gbcam-hdr-utils) by Sébastien Tourneux, take a save made in AEB mode (Auto Exposure Bracketing) with Photo! and turn it into an High Definition Range 8-bits image.
- [gbcamextract](https://github.com/jkbenaim/gbcamextract) by jkbenaim, extracts photos from Game Boy Camera saves.
- [gbcamera](https://github.com/mrdanielps/gbcamera) by mrdanielps, a Python Game Boy Camera dumper.
- [Obscura](https://github.com/svendahlstrand/obscura) by svendahlstrand, salvage your beautiful photographs from a Game Boy Camera save file.
- [gbcamtool](https://github.com/Lana-chan/gbcamtool) by Lana-chan, a web-based tool for extracting pictures out of Game Boy Camera save files.
- [Gameboy.photography](https://github.com/edd/gameboy.photography) by edd, a site that can extract Game Boy camera photos from a save file.
- [gbcamconv](https://github.com/grosshei/gbcamconv) by grosshei, convert a Game Boy Camera save file to png images.
- [gameboy-camera-image-extractor](https://github.com/thies4321/gameboy-camera-image-extractor) by thies4321, a PHP library for extracting Game Boy Camera photos from a save ram (.sav) file and writing them in the PNG format.
...
- [DMG DarkRoom](https://github.com/clickysteve/dmg-darkroom) by clickysteve, [online utility](https://dmgdarkroom.allmyfriendsarejpegs.com/) to extract Game Boy camera saves and process them in interesting ways.
- [GB Storage](https://github.com/MaddoScientisto/GB-Storage) by MaddoScientisto, a Game Boy camera storage system API and frontend built with ASP.NET Core.
- [gbacam](https://github.com/Alexsandr0x/gbacam) by Alexsandr0x, a basic Python project to experiment with real-time access to the memory of the Nintendo Game Boy Camera cartridge.
- [GBxCAM Viewer](https://github.com/tolik518/GBxCAM-Viewer) by tolik518, an Android app to dump photos directly from your Game Boy Camera with a GBxCart, derived from FlashGBX USB protocol implementation.
- [gb_camera](https://github.com/neppermintp/gb_camera) by neppermintp, a tool for managing GameBoy Camera dumps.
...
# Game Boy Printer
## Printer emulators
- The [Arduino Game Boy Printer emulator](https://github.com/mofosyne/arduino-gameboy-printer-emulator) by mofosyne, the most versatile emulator, ultra easy to build, supporting the 110 games compatible with the printer. Many open source decoders available, see the emulator overlays section.
- The [Retrospy Pixel](https://retro-spy.com/product/pixel-gameboy-printer/) is just an Arduino nano flashed with the previous project code. Just move along and [build your own](https://github.com/Raphael-Boichot/Collection-of-PCB-for-Game-Boy-Printer-Emulators) for cheap, it's easy and stays compatible with the decoder provided by Retrospy Technologies.
...
- The [GameBoy Camera Adapter](https://github.com/antoxa2584x/gameboy-camera-adapter) by Anton Artemov, a quite evolved fork of the Pico GameBoy printer going with [its own Android app](https://github.com/antoxa2584x/gameboy-camera-adapter-companion) and a fancy tiny PCB.
...
- The [GBPrinter-DPU411](https://github.com/Vortetty/GBPrinter-DPU411?tab=readme-ov-file) a clone of the arduino Gameboy Printer emulator made to address a printeDPU411 Type 2 printer (or apparently any other thermal printer old enough), along with its [dedicated code fork](https://github.com/Vortetty/arduino-gameboy-printer-parser-for-DPU411TYPE2-printer). The code is now maintained [here](https://codeberg.org/WinterSys/DPU411TYPE2-Gameboy-Printer_Hardware_Pico-Software).
...
- [Game Boy Printer Web Client](https://github.com/GB-Link/gb-printer-web) by GB-Link, Game boy Printer emulator to print from GB hardware to PC, with [GBLink firmware](https://github.com/GB-Link/GBLink-Firmware).
...
- The [GBP-Downloader](https://github.com/HexLions/GBP-Downloader) by HexLions, browser-based tool to receive and save Game Boy Camera photos via Web Serial API — no install, just plug and print !
- A [Javascript interface](https://github.com/Jonic/gbc-printer) to decode the images from the Arduino printer emulator, by Jonic, with [dedicated website](https://gbc-printer.100yen.co.uk/). Accepts only the old packet format from early versions of the Arduino emulator.
...
## Printer custom libraries
- [GBDK-2020 Library for the Gameboy Pocket Printer](https://github.com/T0biasCZe/printlib2) by T0biasCZe. No Readme.md but enough examples for who knows what to do...
...
- [QR Paint](https://bbbbbr.itch.io/game-boy-qr-paint) by bbbbbr, draw on the Game Boy and share it directly using a QR Code or a Game Boy Printer !

As more and more homebrew projects use the printer function thanks to GB Studio dedicated library and GBDK support, I'm probably still behind on the list.
...
- [Compatible thermal paper sold by ZedLabz](https://www.zedlabz.com/products/compatible-printer-paper-roll-for-nintendo-game-boy-printer-self-adhesive-4-pack-white-zedlabz). This is the only known reference with sticking paper, frequently out of stock, untested on my side.
...
- [The Mini Photo Frame](https://www.latente.it/blog/mini-photo-frame-per-gameboy-camera-con-un-esp32-c3-mini-tv-lcd-1-44/), a similar project with an ESP32 C3 Mini TV LCD 1.44.
...
# Social media
 ## Dedicated servers
- [Game Boy Camera Club Discord](https://discord.com/invite/eX3gFPYMNv) (about 1k+ users), a server for Game Boy Camera enthusiasts to share photos, tips and tricks, as well as Game Boy Camera projects and mods.
...
- [GameBoy Camera Telegram Bot](https://github.com/tsMartin/gameboy_camera_control) by tsMartin, a Node.js script that allows you to remotely control video and photo capture from a GameBoy Camera connected via USB.
- [Game Boy Camera+ Mod](https://gbstudiocentral.com/tips/game-boy-camera-mod/) by Martin Cremerius-Gauer, quite comprenhensive guide for this mod.
- [PicNRec for Everyone](https://nyuu.page/projects/picnrec/), by nyuu. A Linux library for interacting with the PicNRec, a CLI interface, and a GUI interface, intead of using native Windows.
- [Digitising Game Boy Camera images, the hard way](http://baccyflap.com/res/gbc/), Super Game Boy, video capture card, Linux, all that shit.
- [Messing with the Arduino emulator](https://www.retrololo.de/2025/07/12/337-arduino-gameboy-printer-emulator/), by Max Mustermann, a guy doing retro stuff and camera/printer related posts.
...
- [Gameboy Camera Club www](https://github.com/gbdev/cameraclub-www), Home page of the Gameboy camera club, showing off some community content and curated zines.
- [Game Boy Camera µZine 2024](https://cameraclub.gbdev.io/uzine2024/) and [2026](https://cameraclub.gbdev.io/uzine2026/), issued by the Game Boy Camera Club Telegram and [gbdev](https://cameraclub.gbdev.io/). To be continued.
...
- [A GB camera photo zine in a cartridge](https://lmkdots.itch.io/160x128x112) by lmk.dots.
 ...
 # What has not (yet) been done if you want to contribute
 - **Disassembly of the Game Boy Camera roms.** Some behaviors of the Camera still remain unclear: how does the Game Boy Camera store the [calibration settings](https://github.com/Raphael-Boichot/Inject-pictures-in-your-Game-Boy-Camera-saves?tab=readme-ov-file#part-3-calibrating-the-sensor) in its SRAM (location of calibration data is known but conversion in registers is not), what is the protocol to exchange images between cameras (few attempts to understand were made but none of them was a success), how does the auto-exposure works (it was indirectly reverse engineered by intercepting [commands sent to sensor](https://github.com/Raphael-Boichot/Play-with-the-Game-Boy-Camera-Mitsubishi-M64282FP-sensor/tree/main/Research%20on%20real%20camera%202024) and integrated into [Photo!](https://github.com/untoxa/gb-photo) but how the code does it for real in the camer ROM is unknown), etc.
 - **Do a flashable Game Boy Camera with SD support because the Analogue Pocket is much too expensive.** It requires a moderate electrical engineering (CPLD typically) but totally doable. Just a matter of people + motivation + skill in the same place.
 - **Close definitively the list of Game Boy Printer compatible games.** To date [110 genuine games](https://github.com/Raphael-Boichot/GameboyPrinterPaperSimulation?tab=readme-ov-file#the-complete-and-tested-list-of-games-compatible-with-the-game-boy-printer-and-the-printer-emulator-%E3%83%9D%E3%82%B1%E3%83%83%E3%83%88%E3%83%97%E3%83%AA%E3%83%B3%E3%82%BF---%E5%AF%BE%E5%BF%9C%E3%82%BD%E3%83%95%E3%83%88) has been found but the list was never officially published by Nintendo and there is still a doubt on the list completeness, putting asside homebrews of course. Typically many games have [printer support in the code](https://github.com/Raphael-Boichot/GameboyPrinterPaperSimulation?tab=readme-ov-file#list-of-games-that-embed-a-printer-library-but-without-printer-support-for-the-player) but no documented way to access it from gameplay nor indication of printer support on the box. Of course I've consulted [tcrf](https://tcrf.net/The_Cutting_Room_Floor) for each game before giving up.
 - Oh, and I search the user manual of [this device](https://github.com/Raphael-Boichot/Teardown-of-the-LaPochee-digital-camera-module) if you have one, even a scan, You will make me an accomplished man.

Jump in Discord or Telegram groups if you want to contribute !

# Your new addiction - getting started
- **What is the best lens mod ?** Without hesitation the [Camera +](https://ko-fi.com/s/9457d1cc6e) or better, the [Camera + Mini](https://ko-fi.com/s/a4d7bd649a) CS lens mods. CS CCTV lenses are ultra common and versatile. The Camera + is a little bit wobbly with heavy lenses but if the Camera + Mini is more stable, it requires a shorter camera PCB like the [GB Mini Camera](https://github.com/Raphael-Boichot/Game_Boy_Mini_Flashable_Camera). Use nylon PA12 to 3D print if you have access to it.
- **What is the best lens to begin with a CS lens mod ?** Without hesitation a 2.8-12 mm CS lens. The actual camera lens is about 3.5 mm with a crop factor of 10 so the 2.8-12 mm starts as a 28 mm equivalent, covers the actual plastic lens (35 mm equivalent) and goes to a 120 mm equivalent zoom (boring but does the job). Large enough range to play with. After that lens, what I can advise is the 5-50 mm CS lens but tripod (or very steady hand) becomes more or less mandatory. Also remind that focusing correctly with the camera dithering is surprisingly difficult. For macro photography, just use the preceding lenses with additional C/CS rings, as much as you want.
- **What are the best Game Boy Printer Emulators ?** Two very good options, the [Pico GB Printer](https://github.com/untoxa/pico-gb-printer) (cheap), or the [TinyGB Printer](https://github.com/Raphael-Boichot/The-TinyGB-Printer) (less cheap but still cheap) derived from the excellent [Arduino version](https://github.com/mofosyne/arduino-gameboy-printer-emulator) you MUST try first in any case. The Pico GB printer has the advantage of featuring ultra fast printing with [Photo!](https://github.com/untoxa/gb-photo) and a remote control option while the TinyGB Printer inheritates the 100% compatility with any known game of the Arduino version, an autonomous recording on SD card and a cool live display showing what you print in real time. You should notice that I'm not a very big fan of ESP32 based devices, this is [why](https://docs.espressif.com/projects/esp-idf/en/latest/esp32/security/vulnerabilities.html) (among [other reasons](https://github.com/Raphael-Boichot/Play-with-the-Game-Boy-Camera-Mitsubishi-M64282FP-sensor/tree/main/ESP32_version_beta)).

To summary, with [Photo!](https://github.com/untoxa/gb-photo) flashed on a [GB Mini Camera](https://github.com/Raphael-Boichot/Game_Boy_Mini_Flashable_Camera) into a [Camera + Mini shell](https://ko-fi.com/s/a4d7bd649a) equiped with a 2.8-12 mm CS lens, you basically have the best possible Game Boy Camera experience. Add a [TinyGB Printer](https://github.com/Raphael-Boichot/The-TinyGB-Printer) or a [Pico GB Printer](https://github.com/untoxa/pico-gb-printer) and you have an easy way to get your images in a digital format directly compatible with all social media (png). Oh, you will probably need a [GBxCart](https://www.gbxcart.com/) also.

Any question, any technical help required ? Jump to the dedidacted [Discord](https://disboard.org/fr/server/568464159050694666) or [Telegram](https://t.me/gameboycamera) groups
