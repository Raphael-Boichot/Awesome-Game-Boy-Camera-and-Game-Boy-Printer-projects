...
**Warning: public Discord servers are famous for being packed with deviant people. Discord is also privacy nightmares.** There's no easy way for users to mass delete their own messages. Moderators can lock them in read-only mode or ban you and cut off any access. Deleting your account does not change anything. There is basically NO support. My advise: **jump in a Discord server ONLY if you can physically reach the moderators.** There is no happy end otherwise.
